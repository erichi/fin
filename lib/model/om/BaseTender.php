...
/**
 * Base class that represents a row from the 'tender' table.
 *
 * 
 *
 * @package    lib.model.om
 */
abstract class BaseTender extends BaseObject  implements Persistent {
	...
	/**
	 * Initializes internal state of BaseTender object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     Tender The current object (for fluent API support)
	 */
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = TenderPeer::ID;
		}
	...
	public function setName($v)
	{
		...
		if ($this->name !== $v) {
			$this->name = $v;
			$this->modifiedColumns[] = TenderPeer::NAME;
		}
	...
	public function setBudget($v)
	{
		...
		if ($this->budget !== $v) {
			$this->budget = $v;
			$this->modifiedColumns[] = TenderPeer::BUDGET;
		}
	...
	public function setAmount($v)
	{
		...
		if ($this->amount !== $v) {
			$this->amount = $v;
			$this->modifiedColumns[] = TenderPeer::AMOUNT;
		}
	...
	public function setStatus($v)
	{
		...
		if ($this->status !== $v || $this->isNew()) {
			$this->status = $v;
			$this->modifiedColumns[] = TenderPeer::STATUS;
		}
	...
	public function setJobOrderId($v)
	{
		...
		if ($this->job_order_id !== $v) {
			$this->job_order_id = $v;
			$this->modifiedColumns[] = TenderPeer::JOB_ORDER_ID;
		}
	...
	public function setBusinessUnitId($v)
	{
		...
		if ($this->business_unit_id !== $v) {
			...
			$this->modifiedColumns[] = TenderPeer::BUSINESS_UNIT_ID;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->job_order_id = ($row[$startcol + 5] !== null) ? (int) $row[$startcol + 5] : null;
			$this->business_unit_id = ($row[$startcol + 6] !== null) ? (int) $row[$startcol + 6] : null;
			$this->resetModified();
			...
			return $startcol + 7; // 7 = TenderPeer::NUM_COLUMNS - TenderPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating Tender object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = TenderPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BaseTender:delete:pre') as $callable)
			{
			...
			if ($ret) {
				TenderPeer::doDelete($this, $con);
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseTender:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseTender:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseTender:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

				TenderPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = TenderPeer::doInsert($this, $con);
					...
					$affectedRows += TenderPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = TenderPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = TenderPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = TenderPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getName(),
			...
			$keys[5] => $this->getJobOrderId(),
			$keys[6] => $this->getBusinessUnitId(),
		);
		return $result;
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[5], $arr)) $this->setJobOrderId($arr[$keys[5]]);
		if (array_key_exists($keys[6], $arr)) $this->setBusinessUnitId($arr[$keys[6]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(TenderPeer::ID)) $criteria->add(TenderPeer::ID, $this->id);
		if ($this->isColumnModified(TenderPeer::NAME)) $criteria->add(TenderPeer::NAME, $this->name);
		if ($this->isColumnModified(TenderPeer::BUDGET)) $criteria->add(TenderPeer::BUDGET, $this->budget);
		if ($this->isColumnModified(TenderPeer::AMOUNT)) $criteria->add(TenderPeer::AMOUNT, $this->amount);
		if ($this->isColumnModified(TenderPeer::STATUS)) $criteria->add(TenderPeer::STATUS, $this->status);
		if ($this->isColumnModified(TenderPeer::JOB_ORDER_ID)) $criteria->add(TenderPeer::JOB_ORDER_ID, $this->job_order_id);
		if ($this->isColumnModified(TenderPeer::BUSINESS_UNIT_ID)) $criteria->add(TenderPeer::BUSINESS_UNIT_ID, $this->business_unit_id);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(TenderPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Tender (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Tender Clone of current object.
	 * @throws     PropelException
	 */
	...
	public function getJobOrder(PropelPDO $con = null)
	{
		if ($this->aJobOrder === null && ($this->job_order_id !== null)) {
			...
			/* The following can be used additionally to
			   ...
			   $this->aJobOrder->addTenders($this);
			 */
		}
	...
	public function getBusinessUnit(PropelPDO $con = null)
	{
		if ($this->aBusinessUnit === null && ($this->business_unit_id !== null)) {
			...
			/* The following can be used additionally to
			   ...
			   undesirable since it could result in an only partially populated collection
			   in the referenced object.
			   $this->aBusinessUnit->addTenders($this);
			 */
		}
	...
	{
	  if (!$callable = sfMixer::getCallable('BaseTender:'.$method))
	  {
	    throw new sfException(sprintf('Call to undefined method BaseTender::%s', $method));
	  }
