...
/**
 * Base class that represents a row from the 'job_type' table.
 *
 * 
 *
...
abstract class BaseJobType extends BaseObject  implements Persistent {
	 ...
	 * @return     JobType The current object (for fluent API support)
	 */
	...
	public function setName($v)
	{
		...
		if ($this->name !== $v) {
			$this->name = $v;
			$this->modifiedColumns[] = JobTypePeer::NAME;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 2; // 2 = JobTypePeer::NUM_COLUMNS - JobTypePeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = JobTypePeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BaseJobType:delete:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseJobType:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseJobType:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseJobType:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

				JobTypePeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(JobTypePeer::NAME)) $criteria->add(JobTypePeer::NAME, $this->name);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of JobType (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     JobType Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 * Otherwise if this JobType has previously been saved, it will retrieve
	 * related Jobs from storage. If this JobType is new, it will return
		...
		if ($this->collJobs === null) {
			if ($this->isNew()) {
			   $this->collJobs = array();
				...
				$criteria->add(JobPeer::JOB_TYPE_ID, $this->id);
	 ...
	 * Otherwise if this JobType is new, it will return
	 * an empty collection; or if this JobType has previously
	 * been saved, it will retrieve related Jobs from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in JobType.
	 */
	public function getJobsJoinJobOrder($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(JobTypePeer::DATABASE_NAME);
		}
		...
		if ($this->collJobs === null) {
			if ($this->isNew()) {
				...
				$this->collJobs = JobPeer::doSelectJoinJobOrder($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	...
	{
	  if (!$callable = sfMixer::getCallable('BaseJobType:'.$method))
	  {
	    throw new sfException(sprintf('Call to undefined method BaseJobType::%s', $method));
	  }
